...
The page is a full-height PWA shell. A compact command panel sits beside or above an iframe-hosted WASM-4 cartridge depending on available width. No landing-page hero or nested cards. On small screens, Enter game opens a play view that hides the web shell chrome and gives the iframe one full-height surface; the cartridge keeps its own title screen, hi score, runtime menu, and native virtual gamepad inside `cart.html`.
...
- Game viewport: iframe-hosted WASM-4 runtime, square in the desktop shell and full-height in mobile play view.
...
- Enter game button: filled accent action, keyboard and pointer accessible.
...
- Mobile controls: the PWA shell uses an iframe so the WASM-4 runtime owns its built-in virtual gamepad, pause menu, and cartridge input without shell-owned event bridging.
